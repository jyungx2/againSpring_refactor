...
function ReviewList({ productId }) {
  ...
  const filteredReviews = data?.filter(
    (review) => review.product._id === parseInt(productId)
  );

  // 각 리뷰의 상품 ID를 콘솔에 출력
  filteredReviews?.forEach((review) => {
    console.log("Product ID in review:", review.product._id);
  });

  const list = filteredReviews?.map((review) => (
    <ReviewItem key={review._id} item={review} currentProductId={productId} />
  ));
