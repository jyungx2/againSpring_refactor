  ...
  const login = useMutation({
    ...
    onError: (err) => {
      ...
      if (err.response?.data.errors) {
        ...
        alert(err.response.data.message || '오류가 발생하였습니다. 잠시 후 다시 요청하세요.');
      }
    },
  });
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:description" content="다시봄에 로그인하고 다양한 혜택과 서비스를 이용하세요." />
      </Helmet>

      <div className="py-32 mx-auto max-w-[1200px] flex flex-col items-center">
        <div className="relative inline-block group">
          <img src="/icons/admin-Icon.png" alt="admin" className="w-10 mx-auto mb-4  animate-blink hover:animate-none" />
          ...
          <div className="w-[320px]">
            <form onSubmit={handleSubmit(login.mutate)}>
              <div className="flex flex-col gap-3">
                <div className={`border-2 border-grey-10 rounded-2xl focus-within:border-secondary-20 px-4 ${errors.email ? `${styles.error}` : ''}`}>
                  <input id="email" type="text" placeholder="이메일" className={`${styles.inputUnset} ${styles.inputCustom}`} {...register('email', { required: '이메일은 필수입니다.' })} />
                </div>
                <ErrorMsg target={errors.email} />

                <div className={`border-2 border-grey-10 rounded-2xl focus-within:border-secondary-20 px-4 ${errors.password ? `${styles.error}` : ''}`}>
                  <input
                    id="password"
                    type="password"
                    placeholder="비밀번호"
              ...
              <div className="mt-8">
                <button className="font-gowunBold w-full h-[42px] rounded-[12px] text-center cursor-pointer box-border bg-primary-40 text-white mb-[10px] hover:bg-primary-30">로그인</button>
                <button
                  type="button"
                ...
                >
                  <img src="/icons/kakao-talk.png" alt="카카오톡" className="w-10 h-10" />
                  카카오톡으로 시작하기
                </button>
                {/* 구글 로그인 추가 */}
                <GoogleLoginButton />
              ...
              <input type="checkbox" id="stay-login" className={`${styles.inputUnset} ${styles.checkboxCustom} peer`} checked={autoLogin} onChange={(e) => setAutoLogin(e.target.checked)} />
              <label htmlFor="stay-login" className="flex items-center gap-3 mt-14 before:content-[''] before:w-[20px] before:h-[20px] before:inline-block before:bg-[url('/icons/emptybox.svg')] peer-checked:before:bg-[url('/icons/checkbox.svg')]">
                자동 로그인
              </label>
