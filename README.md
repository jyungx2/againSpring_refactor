...
2. 상품 상세 페이지 & 장바구니 페이지 - 주문 생성 API(POST /orders) 일시적 오류 및 백엔드 점검 중
  현재 주문 API(/orders)에 일시적인 오류가 발생하여 상품 상세 페이지와 장바구니 페이지의 주문 요청 기능이 잠시 중단된 상태입니다.
  안정적인 서비스 제공을 위해 백엔드 점검이 진행 중이며, 빠른 시일 내 정상화될 예정입니다.

4. 기존 api 서버 도메인 만료로 인한 오류 해결  
